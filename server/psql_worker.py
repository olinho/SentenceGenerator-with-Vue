...
from server.psq_query_builder import QueryBuilder
...
class PsqlWorker:
    ...
    def get_lemko_translation(self, polish_word: str):
        query = "SELECT base_form " \
                "FROM terms " \
                "WHERE polish_translation = %s "
        resp = self.do_query(query, (polish_word,))
        return resp

    def get_word_details(self, word: str, part_of_speech = -1):
        cols = ["base_form", "polish_translation", "grammatical_gender", "grammatical_part_of_speech"]
        cols_as_str = ",".join(cols)
        nextCond = ""
        if part_of_speech != -1:
            nextCond = " AND grammatical_part_of_speech = " + str(part_of_speech) + " "
        query = "SELECT " + cols_as_str + " " \
                "FROM terms " \
                "WHERE LOWER(base_form) = %s " + nextCond + \
                "LIMIT 1"
        resp = self.do_query(query, (word,), cols)
        if len(resp) > 0:
            return resp[0]
        else:
            warnings.warn('Unknown word ' + word)
    ...
    def get_all_part_of_speech(self, word: str):
        resultList = []
        query = "SELECT DISTINCT(grammatical_part_of_speech) FROM terms WHERE base_form = %s"
    ...
    def get_verb_conjugation(self, word: str):
    ...
    def get_base_form_for_word(self, word: str, grammatical_part_of_speech=-1):
        nextCond = ""
        if grammatical_part_of_speech != -1:
            nextCond = " AND grammatical_part_of_speech = " + grammatical_part_of_speech + " "
            grammatical_part_of_speech = str(grammatical_part_of_speech)
        query = "SELECT CASE " \
                "WHEN EXISTS " \
    ...
    def get_sentence_with_word(self, word: str):
        ...
        if len(resultList) == 0:
            warnings.warn('Brak zdań z podanym wyrazem')
            return ""
        else:
            return resultList[0]

    def get_new_words(self, old_words: [], part_of_speech = -1, limit = 10, order = "asc"):
        whereClause = " editing_state = 2 AND polish_translation != '' "
        if len(old_words) > 0:
...
# db = PsqlWorker()
# print(db.get_word_details('єґомость'))